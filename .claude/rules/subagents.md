...
| L2 | Rules | 全量加载（自动） | 22 个 rules 文件 | `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 14 个 skills（项目自建 7 + 第三方安装 5 + 元工具 2） | `.claude/skills/*/SKILL.md` |
...
### 项目自建 Skills（7 个，`.claude/skills/` 目录内）
...
| `graphify-workflow` | `/graphify-workflow [query\|update\|viz]` | 知识图谱导航与可视化 | graphify CLI |
| `context-research` | `/context-research`（context: fork） | 深度代码搜索（隔离上下文） | Explore agent |
| `manual-review` | `/manual-review` | 用户手册截图与文档一致性审查 | Vision MCP |
| `long-screenshot` | 脚本调用 | 长页面滚动截图 | Playwright |
| `create-colleague` | `/create-colleague` | 把同事蒸馏成 AI Skill | 飞书/钉钉数据 |

### 第三方安装 Skills（5 个，symlink 到 `.agents/skills/`）
