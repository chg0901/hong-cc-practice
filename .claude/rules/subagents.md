...
| L2 | Rules | 全量加载（自动） | 33 个 rules 文件 | `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 22 个 skills（项目自建 10 + 第三方 5 + 第三方 symlink 7） | `.claude/skills/*/SKILL.md` |
...
### 项目自建 Skills（10 个，`.claude/skills/` 目录内，同步到 GitHub）
