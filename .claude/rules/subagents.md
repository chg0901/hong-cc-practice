...
| L2 | Rules | 全量加载（自动） | 23 个 rules 文件 | `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 14 个 skills（项目自建 5 + 第三方安装 9） | `.claude/skills/*/SKILL.md` |
...
### 项目自建 Skills（5 个，`.claude/skills/` 目录内，同步到 GitHub）
...
### 第三方安装 Skills（3 个，git clone/npm 安装，不同步）

book2skills, create-colleague, context-research

### 第三方安装 Skills（5 个，symlink 到 `.agents/skills/`，不同步）
