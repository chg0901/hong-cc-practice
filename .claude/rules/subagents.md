...
| L2 | Rules | 全量加载（自动） | 37 个 rules（18 全局 + 19 项目） | `~/.claude/rules/*.md` + `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 26 个 skills（项目自建 14 + 第三方 5 + 第三方 symlink 7） | `.claude/skills/*/SKILL.md` |
...
### 项目自建 Skills（14 个，`.claude/skills/` 目录内，同步到 GitHub）
