...
| L2 | Rules | 全量加载（自动） | 30 个 rules 文件 | `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 16 个 skills（项目自建 8 + 第三方 5 + 第三方 symlink 7） | `.claude/skills/*/SKILL.md` |
...
### 项目自建 Skills（8 个，`.claude/skills/` 目录内，同步到 GitHub）
...
- [2026-04-22] L3 Skills 15→16（第三方 4→5，新增 excalidraw-diagram-generator；github/awesome-copilot，14.6K installs）；fireworks-tech-graph.md 新增三工具分工表（Mermaid/fireworks/Excalidraw）；post_edit_visual_check.sh 新增 .excalidraw 触发
