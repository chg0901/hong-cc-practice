...
必须同时保留**文字版**和**Mermaid 图**。对于窄长图（ratio < 0.5 的纵向流程图），使用 `<img>` 标签控制显示宽度，避免文字过小：

```markdown
<!-- 正常宽高比（0.5-3.0）— 使用标准 markdown -->
![图表标题](../mermaid/figure/xxx.png)

<!-- 窄长图（< 0.5）— 使用 img 标签控制宽度 -->
<img src="../mermaid/figure/xxx.png" width="300"/>
```

**宽度参考**：
- 横向图（ratio > 1.0）：默认 100% 显示或 `width="600"`
- 纵向图（ratio < 0.5）：`width="300"` 到 `width="400"`
- 方形图（0.5-1.0）：默认即可
