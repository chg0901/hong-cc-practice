...
|-------|------|----------|----------|-----------|
| L1 | Hooks | 事件触发（自动） | 6 个 hooks（PreToolUse x2, PostToolUse x2, SubagentStart x1, SubagentStop x1, SessionEnd x1） | `.claude/settings.json` + `~/.claude/settings.json` |
| L2 | Rules | 全量加载（自动） | 38 个 rules（19 全局 + 19 项目） | `~/.claude/rules/*.md` + `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 19 个项目 skills（项目专用 12 + 第三方安装 7） | `.claude/skills/*/SKILL.md` |
...
Foundation: Memory（经验积累，自动记录）-- `memory/*.md`（38 个文件）

## Skills 全景图

### 项目专用 Skills（12 个，`.claude/skills/` 中的 symlinks，指向 `~/.claude/skills/`，同步到 GitHub）
...
| `ps-script-dev` | `/ps-script-dev` | PS 脚本开发工作流 + 踩坑预防 | terminal.md rules |
| `db-optimization` | 自动（"数据库优化/查询慢/索引"）+ `/db-optimization` | SQLite 查询优化 + 结构优化工作流 | database-patterns.md rule |
| `spike` | 自动（"spike, experiment, POC, feasibility"）+ `/spike` | 快速实验工作流（subagent 隔离） | Agent tool + superpowers:writing-plans |
| `ralph` | `/ralph` + 自动（"loop until done", "iterate until"） | 持久迭代循环（完成条件驱动） | Agent tool + test-runner |
| `prd` | `/prd` + 自动（"write PRD", "create PRD", "new feature requirements"） | 结构化产品需求文档工作流 | superpowers:writing-plans |
| `context-audit` | `/context-audit` 或每季度第一天 | 五层体系 Token 预算 + 双重加载审计 | token-budget.md rule |

### 第三方安装 Skills（7 个，`.claude/skills/` 中的 symlinks，指向 `.agents/skills/`，不同步到 GitHub）

| Skill | 来源 | 触发方式 | 用途 | 项目集成建议 |
|-------|------|---------|------|------------|
| `book2skills` | 手动安装 | 手动 | 书籍内容转为 Skills | — |
...
### Superpowers 触发矩阵（13 skills 深度集成）

superpowers 是方法论型 skill 套件，与五层体系深度集成。以下矩阵明确每个 skill 的触发时机和协作链：
...
| PostToolUse (Bash) | 工具执行后 | L2 Rules | 检测 git commit 命令，强制提醒记录 work_summary + changelog |
| SubagentStart | 子代理启动 | L4 Subagents | 注入项目上下文（分支、NO_PROXY、规则摘要） |
| SubagentStop | 子代理完成 | L4 Subagents | 触发后续动作（测试失败修复、文档验证） |
| SessionEnd | 对话结束 | L5 Teams + L2 Rules | 自动提交文档、汇总 TODO、检测未记录 commit |
...
**影响**：子代理是"清洁工具"（隔离噪音），不是"省钱工具"。短任务适合用 subagent，长时间运行的分析任务缓存优势不如主会话。详见 `~/.claude/rules/context-management.md`（全局 rule）。
...
- [2026-04-29] context-audit 修复：L2 Rules 37→38（19+19），L3 Skills 重组为"项目专用 12 + 第三方安装 7"，删除双重加载的 13 个 skills（11 个从全局删除，2 个从项目删除），Memory 文件数 9→38
- [2026-04-28] 规则重组：L2 Rules 37→18 全局 + 19 项目（消除双重加载，节省 ~27,600 token/会话）；加载路径更新为 `~/.claude/rules/*.md` + `.claude/rules/*.md`
- [2026-04-26] L2 Rules 35→37（+cross-model-workflow.md +goal-backward.md）；L3 Skills 24→26（+ralph +prd），项目自建 12→14；subagents.md 补充 Tasks 5-10 完成记录
- [2026-04-26] L2 Rules 33→35（+deviation-handling.md +boris-cherny-tips.md）；L3 Skills 23→24（+spike）；项目自建 11→12
- [2026-04-25] L3 Skills 项目自建 10→11（+db-optimization）；database-patterns.md 新增增量迁移模式 + 查询优化反模式
- [2026-04-25] L3 Skills 项目自建 9→10（+ps-script-dev）；Memory 9→10（+feedback_ps_script_pitfalls.md）
- [2026-04-25] L2 Rules 32→33（+user-manual.md 版本管理规则）
- [2026-04-23] L1 Hooks 5→6（新增 PostToolUse:Bash — git commit 后强制提醒记录 work_summary + changelog）；SessionEnd 增强（检测未记录 commit）
