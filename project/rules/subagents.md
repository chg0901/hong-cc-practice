...
| L2 | Rules | 全量加载（自动） | 39 个 rules（19 全局 + 20 项目） | `~/.claude/rules/*.md` + `.claude/rules/*.md` |
| L3 | Skills | 按需加载（手动/自动） | 20 个项目 skills（项目专用 13 + 第三方安装 7） | `.claude/skills/*/SKILL.md` |
...
### 项目专用 Skills（13 个，`.claude/skills/` 中的 symlinks，指向 `~/.claude/skills/`，同步到 GitHub）
...
| `context-audit` | `/context-audit` 或每季度第一天 | 五层体系全量审计（Token 预算 + 双重加载 + 超限 + 修复指引） | token-budget.md rule |
| `five-layer-add` | `/five-layer-add` 或每次新增五层体系内容后 | 增量健康检查（双重加载/计数/超限/归位/同步） | five-layer-integrity.md rule |
...
→ 完整矩阵见 [`memory/reference_superpowers_matrix.md`](../../memory/reference_superpowers_matrix.md)
...
- [2026-04-29] 新增 five-layer-integrity.md rule（项目 rules 19→20）+ five-layer-add skill（项目专用 12→13）；context-audit SKILL.md 扩充修复指引章节
- [2026-04-29] context-audit 修复：L2 Rules 37→38（19+19），L3 Skills 重组为"项目专用 12 + 第三方安装 7"，删除双重加载的 13 个 skills（11 个从全局删除，2 个从项目删除），Memory 文件数 9→38；Superpowers 触发矩阵移到 Memory
- [2026-04-28] 规则重组：L2 Rules 37→18 全局 + 19 项目（消除双重加载，节省 ~27,600 token/会话）；加载路径更新为 `~/.claude/rules/*.md` + `.claude/rules/*.md`
- [2026-04-26] L2 Rules 35→37（+cross-model-workflow.md +goal-backward.md）；L3 Skills 24→26（+ralph +prd），项目自建 12→14
- [2026-04-25] L3 Skills 项目自建 9→11（+ps-script-dev +db-optimization）；L2 Rules 32→33（+user-manual.md）
- [2026-04-23] L1 Hooks 5→6（新增 PostToolUse:Bash — git commit 后强制提醒记录 work_summary）；SessionEnd 增强
