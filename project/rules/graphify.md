...
### Git-Status-Based 增量更新（节省 token 策略）

利用 `git diff` 精确定位变更文件，避免全量扫描：

```bash
# 1. 检查自上次 commit 以来的代码文件变更
CODE_CHANGES=$(git diff --name-only HEAD -- '*.py' '*.js' '*.ts' '*.go' '*.rs' '*.java' '*.cpp' '*.c' | wc -l)

# 2. 根据变更量选择策略
if [ "$CODE_CHANGES" -eq 0 ]; then
    echo "No code changes — skip rebuild"
elif [ "$CODE_CHANGES" -le 5 ]; then
    echo "Small change ($CODE_CHANGES files) — incremental rebuild (SHA256 cache handles it)"
    # _rebuild_code() 内部用 SHA256 cache，只重新解析变更文件
else
    echo "Large change ($CODE_CHANGES files) — full rebuild recommended"
fi
```
...
|--------|------|------|----------|
| 0 文件 | 跳过 | — | 0s |
| 1-5 文件 | 增量（SHA256 cache） | `_rebuild_code()` | 5-15s |
| 6-20 文件 | 增量（同上，cache 命中率高） | `_rebuild_code()` | 15-30s |
| 20+ 文件 | 全量重建 | `_rebuild_code()` + `gen_graph_viz.py` | 30-60s |
| merge/rebase 后 | 全量重建 + 可视化 | `/graphify-workflow update` + `/graphify-workflow viz` | 60s+ |

**关键优化**：`_rebuild_code()` 的 SHA256 content-addressable cache 已经实现了文件级增量。真正的节省来自**判断是否需要调用**：配置/文档变更不需要重建图谱。

### 验证更新

更新后检查 `graphify-out/GRAPH_REPORT.md` 的日期和节点数（以 GRAPH_REPORT.md 为权威来源）。
...
规模：     2995 nodes / 6336 edges / 188 communities
